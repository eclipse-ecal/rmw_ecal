...
namespace eCAL
{
  namespace rmw
  {
    ...
    rmw_ret_t rmw_get_topic_names_and_types(const char *implementation_identifier, const rmw_node_t *node,
                                            ...
                                            rmw_names_and_types_t *topic_names_and_types)
    {
      ...
      try
      {
        std::transform(topics.begin(), topics.end(), RosArray::Begin(*topic_names_and_types),
                       [no_demangle, allocator](auto &topic) {
                         std::string demangled_name{no_demangle ? topic.name : DemangleTopicName(topic.name)};
                         ...
                         types.data[0] = ConstructCString(topic.type);
